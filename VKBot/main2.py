...
class VKBot:
    ...
    def send_photo_1(self, user_id, message, index):
        # отправка первой фотографии
        self.vk.method('messages.send', {'user_id': user_id,
                                         'access_token': user_token,
                                         'message': message,
                                         'attachment': f'photo{self.person_id(index)}_{self.get_photo_1(self.person_id(index))}',
                                         'random_id': 0})

    def send_photo_2(self, user_id, message, index):
        # отправка второй фотографии
        self.vk.method('messages.send', {'user_id': user_id,
                                         'access_token': user_token,
                                         'message': message,
                                         'attachment': f'photo{self.person_id(index)}_{self.get_photo_2(self.person_id(index))}',
                                         'random_id': 0})

    def send_photo_3(self, user_id, message, index):
        # отправка третьей фотографии
        self.vk.method('messages.send', {'user_id': user_id,
                                         'access_token': user_token,
                                         'message': message,
                                         'attachment': f'photo{self.person_id(index)}_{self.get_photo_3(self.person_id(index))}',
                                         'random_id': 0})

    def find_persons(self, user_id, index):
        self.write_msg(user_id, self.found_person_info(index))
        #self.person_id(index)
        insert_data_seen_users(self.person_id(index), index) #index
        self.get_photos_id(self.person_id(index))
        self.send_photo_1(user_id, 'Фото номер 1', index)
        if self.get_photo_2(self.person_id(index)) != None:
            self.send_photo_2(user_id, 'Фото номер 2', index)
            self.send_photo_3(user_id, 'Фото номер 3', index)
        else:
...
print(photos_id)
for i in range(30):
   bot.send_photo_1(user_id, 'Hello!', i)
Bot(bot, index)
print('Finish')